...
| 音源 | **Salamander Grand Piano 子集**（CC-BY 3.0） | 上游按小三度网格提供样本（C / D♯ / F♯ / A 四音一组），A0–C8 共 27 个，其余音高由采样器实时变速。**格式用 mp3**：实测 27 个 mp3 合计 1.94 MB，落在 ≤ 2 MB 预算内，且解码兼容性全覆盖，无需再备 opus 回退。**须在页面底部署名（Alexander Holm, CC-BY 3.0）** |
...
- 采样合计 1.94 MB（27 个 mp3），且**在开场手势之后才下载**，首屏不背这 2 MB
