...
形容词不可实现，这里全部定死。实现时直接取这些值，不要另猜。

| 名目 | 值 |
|---|---|
| 符号成形（汇聚） | 1.6s，`cubic-bezier(.22, 1, .36, 1)` |
| 符号 → 星点塌陷 | 2.2s，`cubic-bezier(.65, 0, .35, 1)`（先慢后快再稳，像真的失去支撑） |
| 连线逐条亮起 | 每条 120ms 间隔，单条 400ms 淡入 |
| ambient 交叉淡入 | 5.5s `ease-out`（照搬源站） |
| 钉字逐字母揭示 | 单字 700ms，字间 delay 45ms，`translateX` 起始 ±0.4em |
| replay 重播 | 重跑「汇聚 + 塌陷」全程，共 3.8s |
| curl noise 漂移 | 幅度 0.015 世界单位，频率 0.35，与两态插值叠加而非替换 |
| 拖拽阻尼 | 每帧速度 ×0.92，松手后自然停 |

滚动驱动的量走 CSS 变量，与源站同名法：`--copy-opacity`（正文随离场淡出）、
`--title-parallax-y`（标题与星云的视差位移，上限 ±40px）。

## 页面骨架

`starmap.html` 与其余四个入口同构：

- `lang="zh-CN"`、`<meta name="theme-color" content="#000000">`
- 内联 SVG data-uri favicon：黑底 + 一颗白星 + 一条淡连线（与深渊那枚琥珀圆点区分）
- `<title>` 与 `<meta name="description">` 讲星图隐喻，不复制深渊的文案
- 首个可聚焦元素是 skip-link「跳过星图，直达记忆」，指向第一个星座的内容区
- 其余四个主题的互链条目按现有 `.theme-link` 惯例加，本页也加回链

## 可访问性

- canvas `aria-hidden="true"`——它是背景，不承载信息。所有星座名、年份、记忆正文
  都在 DOM 里，读屏器按文档顺序就能读完一生，不依赖任何特效
- `dragSurface` 是 `<button>`，有 `aria-label`「拖动或用方向键旋转星图」；
  它只改视角、不改内容，因此不需要 live region
- 记忆卡片是 `<article>`，标题 `<h3>`，星座标题 `<h2>`，单一 `<h1>` 给站主名字
- 连线与星点仅作装饰，静态降级下的 SVG 同样 `aria-hidden`
- 焦点可见样式沿用源站规格：`outline: .125rem solid rgba(255,255,255,.7)`，
  `outline-offset: -.25rem`

## 响应式

- **窄屏（< 768px）**：钉字不再左右分立——照源站做法上下错开
  （`labelLeft` 落 46%、`labelRight` 落 54%，贴边 `1rem`）
- 星云容器 `max-width: 36rem`、高 `80vh`（源站 `shapeCue` 规格）；
  窄屏改为宽度满屏、高 `60vh`，给卡片留出读的空间
- 记忆卡片窄屏单列贴底，宽屏靠一侧，永不压在星云正中的过曝核心上
- 总页长 = 开场 100vh + 星座数 × 200vh。星座控制在 4–7 座；
  超过 7 座页面过长，自检里给出警告（不是报错——那是站主的判断）
...
- 星座数 > 7 → 打印警告（不报错。页面会很长，但那是站主的判断，不是错误）
